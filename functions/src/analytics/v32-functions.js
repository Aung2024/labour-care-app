...
const RECONCILIATION_JOB_COLLECTION_V32 = 'analytics_v32_jobs'
const RECONCILIATION_JOB_ID_V32 = 'dashboard-summary-reconciliation'
...
const reconciliationJobRefV32 = (database) => (database || db())
  .collection(RECONCILIATION_JOB_COLLECTION_V32)
  .doc(RECONCILIATION_JOB_ID_V32)

const shouldStartReconciliationV32 = (job, now) => {
  if (!job) return true
...
const startAnalyticsV32Reconciliation = async (
  database,
  requestedBy,
  now,
  force
  ...
  const ref = reconciliationJobRefV32(firestore)
  ...
  return firestore.runTransaction(async (transaction) => {
    ...
    if (!force && !shouldStartReconciliationV32(previous, startedAt.getTime())) {
      return {
    ...
    transaction.set(ref, {
      type: 'analytics-v32-reconciliation',
      status: 'running',
      generation,
      cursor: null,
      processedPatients: 0,
...
const processAnalyticsV32ReconciliationBatch = async (
  database,
  now,
  options
) => {
  ...
  const results = await mapWithConcurrency(
    ...
    async (patient) => {
      try {
        await refreshPatientAnalyticsV32Only(
          firestore,
          patient.id,
        ...
        logger.error('Analytics-v32 reconciliation patient failed', {
...
const processAnalyticsV32ReconciliationUntilDeadline = async (
  database,
  now,
  maxRuntimeMs = 480000
) => {
  ...
  while (Date.now() - started < maxRuntimeMs) {
    result = await processAnalyticsV32ReconciliationBatch(database, now)
...
const startDashboardV32Reconciliation = onCall({
  region: REGION,
  timeoutSeconds: 60,
  memory: '256MiB',
  ...
  return startAnalyticsV32Reconciliation(
    db(),
    request.auth.uid,
    new Date(),
    true
...
const dashboardV32ReconciliationWorker = onSchedule({
  ...
  if (!current || current.status !== 'running') {
    await startAnalyticsV32Reconciliation(
      firestore,
      '48-hour-scheduler',
      new Date(),
      false
  ...
  return processAnalyticsV32ReconciliationUntilDeadline(
    firestore,
    new Date()
  )
})
...
module.exports = {
  ...
  RECONCILIATION_JOB_ID_V32,
  RECONCILIATION_INTERVAL_MS,
  RECONCILIATION_BATCH_SIZE,
  RECONCILIATION_CONCURRENCY,
  reconciliationJobRefV32,
  shouldStartReconciliationV32,
  startAnalyticsV32Reconciliation,
  processAnalyticsV32ReconciliationBatch,
  processAnalyticsV32ReconciliationUntilDeadline,
  startDashboardV32Reconciliation,
  dashboardV32ReconciliationWorker
}
