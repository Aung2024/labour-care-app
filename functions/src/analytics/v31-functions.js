...
const { rebuildJointCareIndex } = require('./joint-care-index')

const REGION = 'us-central1'
const RECONCILIATION_JOB_COLLECTION_V31 = 'analytics_v31_jobs'
const RECONCILIATION_JOB_ID_V31 = 'dashboard-summary-reconciliation'
...
const reconciliationJobRefV31 = (database) => (database || db())
  .collection(RECONCILIATION_JOB_COLLECTION_V31)
  .doc(RECONCILIATION_JOB_ID_V31)

const shouldStartReconciliationV31 = (job, now) => {
  if (!job) return true
  if (job.status === 'running') return false
  const completed = Number(job.lastCompletedAtMillis || 0)
  return !completed || (now || Date.now()) - completed >=
    RECONCILIATION_INTERVAL_MS
}

const startAnalyticsV31Reconciliation = async (
  database,
  requestedBy,
  now,
  force
  ...
  const ref = reconciliationJobRefV31(firestore)
  ...
  return firestore.runTransaction(async (transaction) => {
    ...
    if (!force && !shouldStartReconciliationV31(
      previous,
      startedAt.getTime()
    )) {
      return {
    ...
    transaction.set(ref, {
      type: 'analytics-v31-reconciliation',
      status: 'running',
      generation,
      cursor: null,
      processedPatients: 0,
...
const ensureJointCareIndex = async (firestore, ref, job) => {
  if (job.jointCareIndexReady) return null
  const result = await rebuildJointCareIndex(firestore)
  await ref.set({
    jointCareIndexReady: true,
    jointCareIndexResult: result,
    updatedAt: FieldValue.serverTimestamp()
  }, { merge: true })
  return result
}

const processAnalyticsV31ReconciliationBatch = async (
  database,
  now,
  options
) => {
  const firestore = database || db()
  const ref = reconciliationJobRefV31(firestore)
  let jobSnapshot = await ref.get()
  if (!jobSnapshot.exists || jobSnapshot.get('status') !== 'running') {
    return { status: 'idle' }
  }
  let job = jobSnapshot.data()
  await ensureJointCareIndex(firestore, ref, job)
  if (!job.jointCareIndexReady) {
    jobSnapshot = await ref.get()
  ...
  const results = await mapWithConcurrency(
    ...
    async (patient) => {
      try {
        await refreshPatientAnalyticsV31Only(
          firestore,
          patient.id,
        ...
        logger.error('Analytics-v31 reconciliation patient failed', {
...
const processAnalyticsV31ReconciliationUntilDeadline = async (
  database,
  now,
  maxRuntimeMs = 480000
) => {
  ...
  while (Date.now() - started < maxRuntimeMs) {
    result = await processAnalyticsV31ReconciliationBatch(database, now)
...
const startDashboardV31Reconciliation = onCall({
  region: REGION,
  timeoutSeconds: 60,
  memory: '256MiB',
  ...
  return startAnalyticsV31Reconciliation(
    db(),
    request.auth.uid,
    new Date(),
    true
...
const dashboardV31ReconciliationWorker = onSchedule({
  ...
  if (!current || current.status !== 'running') {
    await startAnalyticsV31Reconciliation(
      firestore,
      '48-hour-scheduler',
      new Date(),
      false
  ...
  return processAnalyticsV31ReconciliationUntilDeadline(
    firestore,
    new Date()
  )
})
...
module.exports = {
  ...
  RECONCILIATION_JOB_ID_V31,
  reconciliationJobRefV31,
  shouldStartReconciliationV31,
  startAnalyticsV31Reconciliation,
  processAnalyticsV31ReconciliationBatch,
  processAnalyticsV31ReconciliationUntilDeadline,
  startDashboardV31Reconciliation,
  dashboardV31ReconciliationWorker
}
